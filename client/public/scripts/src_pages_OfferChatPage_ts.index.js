"use strict";
(self["webpackChunkfront_end"] = self["webpackChunkfront_end"] || []).push([["src_pages_OfferChatPage_ts"],{
...
/* harmony import */ var _services_UserService__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../services/UserService */ "./src/services/UserService.ts");
...
class OfferChatPageController {
    ...
    async initialize() {
        ...
        try {
            // Fetch data
            chat = await _services_ChatService__WEBPACK_IMPORTED_MODULE_1__["default"].getByID(this.chatID);
            if (!chat)
                throw new Error("chat not found");
            this.model.offer = await _services_OfferService__WEBPACK_IMPORTED_MODULE_2__["default"].getByID(chat.offerID);
            if (!this.model.offer)
                throw new Error("offer not found");
            this.model.seller = await _services_UserService__WEBPACK_IMPORTED_MODULE_3__["default"].getByID(this.model.offer.sellerID);
    ...
    async onSubmitMessage(event) {
        ...
        await _services_ChatService__WEBPACK_IMPORTED_MODULE_1__["default"].sendMessage(this.model.chat._id, message, this.currentUser);
        form.reset();
    }
    async keepUpdated() {
        let newChat;
        try {
            newChat = await _services_ChatService__WEBPACK_IMPORTED_MODULE_1__["default"].getByID(this.chatID);
        }
        catch (error) {
            console.error(`Error updating chat: ${error}`);
        }
        const oldMessagesCount = this.model.chat.messages.length;
        const newMessagesCount = newChat.messages.length;
        if (newMessagesCount > oldMessagesCount) {
            const messagesToAdd = newChat.messages.slice(oldMessagesCount).map(msg => {
                return {
                    isMine: msg.senderID === this.currentUser._id,
                    content: msg.content,
                    timestamp: msg.timestamp
            ...
            this.view.update(messagesToAdd);
        }
        this.model.chat = {
            _id: newChat._id,
            offerID: newChat.offerID,
...
//# sourceMappingURL=src_pages_OfferChatPage_ts.index.js.map